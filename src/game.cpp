...
void Game::Run()
{
    while (!WindowShouldClose() && !CheckVictory(m_board, c_states, m_player) && !CheckDraw(m_board, c_states, m_player))
    {
        this->Update();
        this->Draw();
    }

    if (CheckVictory(m_board, c_states, m_player))
    {
        m_text = "Player " + std::to_string(m_player) + " has won!";
        DrawGameOverScreen(m_text, m_bgColor, m_board, m_grid, c_states, m_texBoard, m_texCross, m_texCircle);
    }
    else if (CheckDraw(m_board, c_states, m_player))
        DrawGameOverScreen("Draw!", m_bgColor, m_board, m_grid, c_states, m_texBoard, m_texCross, m_texCircle);
}
...
void Game::Draw()
{
    BeginDrawing();
        ...
        DrawBoard(m_board, m_grid, c_states, m_texBoard, m_texCross, m_texCircle);
